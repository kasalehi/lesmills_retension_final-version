...
def filter_classes_1_2(data_paths: dict):
    ...
    Save as df_merged_2024.parquet
    """
    try:
        ...
        # Save filtered data
        output_path = DATA_DIR / "df_merged_2024.parquet"
        df_filtered.to_parquet(output_path, index=False)

        print(f"✅ Filtered 2024 data: {len(df_filtered)} rows (classes 1 & 2)")
        print(f"📦 Saved to {output_path}")

        return {"df_merged_2024_path": str(output_path)}

    except Exception as e:
        raise Exception(f"Filtering 2024 data failed: {e}")
...
# ================================
with DAG(
    dag_id="PrepareData2024",
    description="Prepare balanced training data for 2024-01-01 (classes 1 & 2)",
    schedule=None,
    start_date=datetime(2024, 1, 1),
    catchup=False
) as dag:

    # Read data for 2024-01-01
    @task
    def read_data_task():
